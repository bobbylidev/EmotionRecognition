...
while True:
    ...
    if result is not None:
        ...
        for index, emotion in enumerate(EMOTIONS):
            cv2.putText(frame, emotion, (10, index * 20 + 20), cv2.FONT_HERSHEY_DUPLEX, 0.5, (30,30,28), 1);
	    cv2.putText(frame, str(int(result[0][index] * 100))+"%", (100, index * 20 + 20), cv2.FONT_HERSHEY_DUPLEX, 0.5, (30,30,28), 1);       

        # find the emotion with maximum probability and display it
        maxindex = np.argmax(result[0])
        font = cv2.FONT_HERSHEY_DUPLEX
        cv2.putText(frame,EMOTIONS[maxindex],(10,360), font, 2,(255,59,0),2,cv2.LINE_AA) 
